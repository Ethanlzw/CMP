...
nx = 16;
ny = 8;
nTriCols = 4;   % now mixed Tri3 + Quad4 works if Tri3_ThermoMech.m is present
...
%% Essential BCs
% This benchmark prescribes the exact free-expansion field on boundary nodes.
essBC = {};
for j = 0:ny
    for i = 0:nx
...
strain_err_max = max(abs(strain_FE(:) - strain_exact(:)));
strainTh_err_max = max(abs(strainTh_FE(:) - strainTh_exact(:)));
stress_err_max = max(abs(stress_FE(:) - stress_exact(:)));
...
%% Plot only if plotting utilities are available
if exist('plotMesh','file') == 2
    plotMesh(model, 'Thermomechanical Free Expansion Mesh', false, false);
end

if exist('plotNodalScalar','file') == 2
...
if exist('plotDeformedMeshContour','file') == 2
